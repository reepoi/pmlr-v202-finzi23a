...
class Diffusion(object):
    ...
    def dynamics(cls, score_fn, x, t):
        ...
        return cls.f(t) * x - 0.5 * cls.g2(t) * score_fn(x, t)
...
class VarianceExploding(Diffusion):
    ...
    @classmethod
    def dsigma(cls, t):
        # Taos: todo: write this analytically?
        return jax.vmap(grad(lambda s: cls.sigma(s)))(t)

    @classmethod
    def velocity_conditional(cls, t, x_noise, x_data):
        sigma_min = 1e-3
        # Taos: optimal transport conditional
        return x_data - (1 - sigma_min) * x_noise
        # Taos: diffusion conditional
        # return unsqueeze_like(x, cls.dsigma(1 - t) / cls.sigma(1 - t)) * (x - x_data)
...
def train_flow_matching(
    ...
    log.info('Param count: %(param_count).2f M', count_params(params['params']) / 1e6)
